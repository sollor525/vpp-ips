/*
 * Copyright (c) 2023 VPP IPS Multi-Content Parser
 *
...
parse_multi_content_rule_options (char *options_str, ips_rule_t *rule)
{
  ...
  while (token)
  {
    ...
    /* Add all other rule options parsing from original parser... */
 ...
 */
int
parse_multi_content_rule_line (char *line, ips_rule_t *rule)
{
